...
GPT-4o reads the article and writes the image prompts itself — no hardcoded layouts.

  1. architecture-diagram.png  — pure system architecture diagram
  2. concept-explainer.png     — step-by-step explainer of key concepts

Both saved into the pipeline folder alongside all other outputs.

Usage:
  python generate_images.py output/pipeline/2026-04-23_netflix-system-design-ai-pm-guide/
  python generate_images.py output/articles/2026-04-23_netflix-system-design-ai-pm-guide.md
"""
...
BRAND       = "aiskillshub.io · Ashima Malik"
...
def generate_prompts(article_text: str, client: openai.OpenAI) -> dict:
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""
You are a professional visual designer creating two images for a technical article.
...
──────────────────────────────────────────
Write a prompt for a pure technical architecture diagram.
- Show all the real system components described in the article and how they connect
- Boxes, arrows, labels — precise and accurate to the article content
- Color-code different layers or component types
- Choose the size that best fits the architecture shape (wide = landscape, tall = portrait)
- TYPOGRAPHY: large bold node labels (18pt+ equivalent), high-contrast text on every box
- Keep label text SHORT (1–3 words per node) so nothing is cramped or illegible
...
Model it exactly after a research paper one-pager / cheat sheet like the GPT-1 summary poster.
...
- Include every important chart, diagram, flow, or plot described in the article
  (if the article doesn't provide one explicitly, design an appropriate one based on the content)
- Cover every key concept thoroughly — motivation, method, architecture, results, analysis, takeaways
...
def main():
    ...
    arch_prompt = data["architecture"]["prompt"]
    arch_size   = data["architecture"]["size"]
    ...
    print(f"  Architecture size: {arch_size}")
    print(f"  Explainer size:    {exp_size}\n")

    # Step 2: Architecture diagram
    print("Step 2/3 — Generating architecture-diagram.png...")
    arch_bytes = upscale_image(generate_image(arch_prompt, client, arch_size))
    arch_file  = output_dir / "architecture-diagram.png"
    arch_file.write_bytes(arch_bytes)
    print(f"  → {arch_file}\n")

    # Step 3: Concept explainer
    ...
    arch_out_size = "×".join(str(int(d) * 2) for d in arch_size.split("x"))
    exp_out_size  = "3072×2048"

    print("Done!")
    print(f"  ├── architecture-diagram.png  ({arch_size} → {arch_out_size} after upscale)")
